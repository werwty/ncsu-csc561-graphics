            ...
            if (numLongSteps % 2 != 0)
                ...
                for (var latAngle = -latLimitAngle; latAngle <= latLimitAngle; latAngle += angleIncr) {
                    ...
                    for (var longAngle = 0; longAngle < 2 * Math.PI + angleIncr; longAngle += angleIncr) { // for each long
                        ...
                        ellipsoid_uv.push(1 - (longAngle / (Math.PI * 2)), 1 - ((.5 * Math.PI + latAngle) / Math.PI));

                    }
                } // end for each latitude
                ellipsoidVertices.push(0, 1, 0); // add north pole
                ellipsoid_uv.push(0, 1)
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet = 0; whichSet < numTriangleSets; whichSet++) { // for each tri set
                ...
                inputTriangles[whichSet].textureCoord = [];
                ...
                for (whichSetVert = 0; whichSetVert < numVerts; whichSetVert++) { // verts in set
                    ...
                    tri_uv = inputTriangles[whichSet].uvs[whichSetVert];
                    inputTriangles[whichSet].textureCoord.push(1 - tri_uv[0], 1 - tri_uv[1]);
                ...
                // Triangle Texture
                textureCoordBuffer[whichSet] = gl.createBuffer()
                gl.bindBuffer(gl.ARRAY_BUFFER, textureCoordBuffer[whichSet]);
                gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(inputTriangles[whichSet].textureCoord), gl.STATIC_DRAW);

                inputTriangles[whichSet].loaded_texture = loadTexture(inputTriangles[whichSet].material.texture);
            ...
            if (inputEllipsoids == String.null)
                ...
                for (var whichEllipsoid = 0; whichEllipsoid < numEllipsoids; whichEllipsoid++) {
                    ...
                    // ellipsoid texture
                    textureCoordBuffer.push(gl.createBuffer());
                    gl.bindBuffer(gl.ARRAY_BUFFER, textureCoordBuffer[textureCoordBuffer.length - 1]);
                    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(ellipsoidModel.texture_coords), gl.STATIC_DRAW);
                    inputEllipsoids[whichEllipsoid].loaded_texture = loadTexture(ellipsoid.texture);
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                shaderProgram.modulate = gl.getUniformLocation(shaderProgram, "modulate");
...
function renderModels() {
    ...
    for (var whichTriSet = 0; whichTriSet < numTriangleSets; whichTriSet++) {
        ...
        //texture
        gl.bindBuffer(gl.ARRAY_BUFFER, textureCoordBuffer[whichTriSet]);
        gl.vertexAttribPointer(shaderProgram.textureCoord, 2, gl.FLOAT, false, 0, 0);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, inputTriangles[whichTriSet].loaded_texture);
        gl.uniform1i(shaderProgram.samplerUniform, 0);

        // which mode of rendering
        gl.uniform1f(shaderProgram.modulate, modulate);


        var alpha = inputTriangles[whichTriSet].material.alpha
    ...
    for (var whichEllipsoid = 0; whichEllipsoid < numEllipsoids; whichEllipsoid++) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, textureCoordBuffer[numTriangleSets + whichEllipsoid]);
        gl.vertexAttribPointer(shaderProgram.textureCoord, 2, gl.FLOAT, false, 0, 0);
        gl.activeTexture(gl.TEXTURE0);


        gl.bindTexture(gl.TEXTURE_2D, inputEllipsoids[whichEllipsoid].loaded_texture);
        gl.uniform1i(shaderProgram.samplerUniform, 0);

        // which mode of blending?
        gl.uniform1f(shaderProgram.modulate, modulate);

        var alpha = inputEllipsoids[whichEllipsoid].alpha
        gl.uniform1f(shaderProgram.alphaUniform, alpha);
...
function loadTexture(texture_name) {
    ...
    // temporarily single pixel until texture is done downloading
    const level = 0;
    ...
    texture.image.onload = function () {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(gl.TEXTURE_2D, level, internalFormat,
            srcFormat, srcType, texture.image);
    ...
    var url = "https://ncsucgclass.github.io/prog4/" + texture_name;
